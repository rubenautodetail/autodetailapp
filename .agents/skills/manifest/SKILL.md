...
```bash
Codex plugins install manifest
```
...
```bash
Codex config set plugins.entries.manifest.config.apiKey "USER_API_KEY"
```
...
```bash
Codex config set plugins.entries.manifest.config.endpoint "USER_ENDPOINT"
```

### Step 5 — Start the gateway

```bash
Codex gateway install
```
...
```bash
grep "manifest" ~/.Codex/logs/gateway.log | tail -5
```
...
| Duplicate OTel registration | Disable the conflicting built-in plugin: `Codex plugins disable diagnostics-otel` |
