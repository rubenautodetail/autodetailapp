---
name: Codex-monitor
description: Monitor de performance do Codex e sistema local. Diagnostica lentidao, mede CPU/RAM/disco, verifica API latency e gera relatorios de saude do sistema.
risk: safe
source: community
...
# Codex Monitor — Diagnóstico de Performance

## Overview

Monitor de performance do Codex e sistema local. Diagnostica lentidao, mede CPU/RAM/disco, verifica API latency e gera relatorios de saude do sistema.
...
- When the user mentions "Codex lento" or related topics

## Do Not Use This Skill When

- The task is unrelated to Codex monitor
- A simpler, more specific tool can handle the request
- The user needs general-purpose assistance without domain expertise

## How It Works

Skill para diagnosticar e resolver problemas de lentidão no Codex e no sistema.
Determina se o gargalo é local (PC) ou remoto (API Codex) e sugere ações corretivas.

## Quando Usar

- Usuário reclama que o Codex está lento ou travando
- Troca de sessões de conversa demora para carregar
- Respostas do Codex demoram muito
- PC parece lento enquanto usa o Codex
...
```bash
python C:\Users\renat\skills\Codex-monitor\scripts\health_check.py
```
...
- **Codex**: Processos e RAM consumida
- **Disco**: Espaço livre. <10% = impacto em swap/performance
- **Rede**: Latência ao endpoint da API Codex
...
```bash
python C:\Users\renat\skills\Codex-monitor\scripts\health_check.py --browsers-detail
```
...
```bash
python C:\Users\renat\skills\Codex-monitor\scripts\monitor.py --interval 30 --duration 300
```
...
## 5. Benchmark Da Api Codex (Opcional)

Para testar se a lentidão é da API:

```bash
python C:\Users\renat\skills\Codex-monitor\scripts\api_bench.py
```

Mede o tempo de resposta do processo Codex local (não faz chamadas à API).
...
- **Codex é pesado**: Ele roda vários processos Electron. É normal consumir 3-5 GB.
