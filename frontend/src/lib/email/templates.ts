...
export function payoutConfirmationTemplate(data: PayoutEmailData): string {
  ...
  const body = `
    ...
    <div style="background:#f9fafb;border-left:4px solid #D0B078;padding:20px;margin:20px 0;border-radius:4px;">
      ...
      <div style="display:flex;justify-content:space-between;margin:8px 0;"><span style="font-weight:600;color:#6b7280;">${isEs ? 'Total facturado' : 'Total billed'}:</span><span>$${Number(data.grossAmount).toFixed(2)}</span></div>
